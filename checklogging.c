...
int html;
{
    ...
    if ((system("test -r /usr/bin/lastlog")) == 0)
    {
        /* run lastlog */
	shellcode = "/usr/bin/lastlog 2>/dev/null >/tmp/lsat1.lsat";
	header = "This is the list of users who have logged into the system\n the Port they logged into, from where, and the last date they did so.\n";
	if ((dostuff(tmp_file, filename, shellcode, header, html)) < 0)
	{
	    /* something went wrong */
	    perror(" Creation of list failed.");
