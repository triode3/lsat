...
int main(int argc, char *argv[])
{
    ...
    */ 
	...
	if ((system("cat /etc/*-release |grep DISTRIB_ID |grep Ubuntu 1>/dev/null 2>/dev/null >/dev/null")) == 0)
	{
	    /* we are pretty sure we are on Ubuntu */
	    distribution = 7;
	}
	else if ((system("cat /etc/*-release |grep DISTRIB_ID |grep LinuxMint 1>/dev/null 2>/dev/null >/dev/null")) == 0)
	{
	...
	if ((system("cat /etc/*-release |grep -i gentoo 1>/dev/null 2>/dev/null >/dev/null")) == 0)
 	{
		/* we are on gentoo */
	    distribution = 4;
	}
